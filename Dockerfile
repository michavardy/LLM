...
RUN pip install transformers pandas tqdm
...
# Run initialization code to create the pipe object and store it as an environmental variable
RUN python -c "from transformers import pipeline; import torch; import os; pipe = pipeline('text-generation', model='HuggingFaceH4/zephyr-7b-beta', torch_dtype=torch.bfloat16, device_map='auto'); os.environ['PIPELINE'] = str(pipe)"
